...
impl<'config, P: Preset> Incremental<'config, P> {
    ...
    pub fn finish(
        ...
        // [New in Gloas:EIP7732] There is no execution payload envelope for the genesis block, so
        // the genesis block is empty.
        if let Some(state) = beacon_state.post_gloas_mut() {
            *state.latest_block_hash_mut() = eth1_block_hash;
        }
