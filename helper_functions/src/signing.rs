...
impl<P: Preset> SignForSingleFork<P> for VoluntaryExit {
    ...
    fn signing_root_from_fork_info(&self, config: &Config, fork_info: ForkInfo) -> H256 {
        ...
        // EIP-7044 pins the domain to Capella, keyed on the fork as in `process_voluntary_exit`.
...
impl<P: Preset> SignForSingleFork<P> for ProposerPreferences {
    ...
    fn signing_root_from_fork_info(&self, config: &Config, fork_info: ForkInfo) -> H256 {
        let epoch = <Self as SignForSingleFork<P>>::epoch(self);
        let domain_type = <Self as SignForSingleFork<P>>::DOMAIN_TYPE;
        // The proposal epoch's fork, even when signed from a state before the fork.
