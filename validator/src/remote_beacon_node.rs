...
impl RemoteBeaconNode {
    ...
    pub async fn refresh_head(&self, slot: Slot) {
        ...
        // A dead stream then costs a request per slot rather than the slot's duties.
        ...
        match self.head_header().await {
            ...
            Ok((head_slot, block_root)) => {
                if !self
                    .chain_head
                    .update_seen_at(asked_at, head_slot, block_root)
                {
                    debug_with_peers!(
                        "{} answered with head {block_root:?} at slot {head_slot} after the \
                         stream delivered a newer one",
                        self.url,
                    );
                }
            }
    ...
    pub(crate) async fn fresh_head_block_root(&self, slot: Slot) -> Result<H256> {
        ...
        {
            debug_with_peers!(
                "{} answered with head {block_root:?} at slot {head_slot} after the stream \
    ...
    fn attestation_publish_timeout(&self, phase: Phase) -> Duration {
        // The rest of the slot: the next proposer can still include it, and a node busy verifying
        // a block may need longer than the aggregate window to accept it.
        self.window_timeout(
    ...
    fn payload_attestation_timeout(&self) -> Duration {
        // Gossip only accepts the vote until the slot ends.
        self.window_timeout(self.chain_config.payload_attestation_due_bps, SLOT_END_BPS)
    }

    fn head_timeout(&self) -> Duration {
        // Polled without a preset in scope, so the tighter of the two schedules applies.
    ...
    fn window_timeout(&self, from_bps: u64, until_bps: u64) -> Duration {
        // A due point is the tick at which a duty runs, as the spec names it, not a deadline.
        self.chain_config
            .slot_component_duration(until_bps.saturating_sub(from_bps))
            .max(MIN_TIMEOUT)
    }

    fn endpoint(&self, path: &str) -> Result<RedactingUrl> {
        // Appended rather than joined, so a node behind a proxy keeps its path prefix and query.
        ...
        let query = url.query().into_iter().chain(query).join("&");

        url.set_query((!query.is_empty()).then_some(query.as_str()));
        url.as_str().parse().map_err(Into::into)
    }
    ...
    pub async fn attester_duties(
        ...
        // Epoch-shaped because `getAttesterDuties` is; the built-in node answers per slot.
...
impl<P: Preset> BeaconNodeApi<P> for RemoteBeaconNode {
    ...
    async fn publish_singular_attestations(
        ...
        // `combined::Attestation` serializes untagged, so this is a `SingleAttestation` from
        // Electra on and a phase 0 `Attestation` before it, exactly as the endpoint expects.
        ...
        let response = self
            ...
            .header(ETH_CONSENSUS_VERSION, phase.as_ref())
            .json(&bodies)
            .timeout(self.attestation_publish_timeout(phase))
            .send()
            .await?;
...
fn saturating_boost_factor(builder_boost_factor: Uint256) -> u64 {
    // Saturates at the value the spec reserves for preferring builders outright.
...
fn produce_block_v4_path(
    ...
    // Always requested, so that a self-built block can be published through any node.
...
mod tests {
    ...
    // A node behind a proxy is reached under its path prefix, with its own query kept.
    #[test_case(
        "http://localhost:5052",
        "/eth/v1/node/syncing",
    ...
    #[test_case(
        "https://proxy/beacon",
        "/eth/v1/node/syncing",
        "https://proxy/beacon/eth/v1/node/syncing";
        "prefix"
    )]
    #[test_case(
        "https://proxy/beacon/",
    ...
    #[test_case(
        "https://proxy/beacon/?key=k",
        "/eth/v1/events?topics=head",
        "https://proxy/beacon/eth/v1/events?key=k&topics=head";
        "prefix with query"
    )]
    fn endpoint_keeps_the_prefix_and_query_of_the_node_url(
        url: &str,
        path: &str,
        expected: &str,
    ) -> Result<()> {
        let node = test_node(ChainConfig::mainnet(), url)?;

        assert_eq!(node.endpoint(path)?.into_url().as_str(), expected);
    ...
    fn timeouts_follow_the_due_points_of_the_phase() -> Result<()> {
        ...
        // Publishing an attestation may take the rest of the slot, not just until aggregation.
        assert_eq!(
            node.attestation_publish_timeout(Phase::Electra),
            Duration::from_micros(8_000_400),
        );
        assert_eq!(
            node.attestation_publish_timeout(Phase::Gloas),
    ...
    // Nodes take the factor as a `uint64`; `prefer_builder_proposals` configures `Uint256::MAX`.
    #[test]
