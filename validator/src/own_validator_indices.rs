...
impl OwnValidatorIndices {
    ...
    pub fn sorted(&self) -> Vec<ValidatorIndex> {
        self.load()
            ...
            .flatten()
            .copied()
            // Ascending, so that caches keyed by the list see the same list each slot.
            .sorted()
            .collect_vec()
    }
    ...
    pub async fn resolve_at_finalized_epoch<P: Preset, W: Wait + Sync>(
        ...
        // Only a finalization can make a deposit's validator appear in the finalized state.
        if self
            .finalized_epoch
    ...
    pub async fn resolve<P: Preset, W: Wait + Sync>(&self, chain_source: &ChainSource<P, W>) {
        ...
        let to_resolve = self
            .load()
            .iter()
            // Indices come from the finalized state, so a resolved key is never asked again.
