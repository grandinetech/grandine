...
use helper_functions::{misc, predicates, signing::SignForSingleFork};
...
impl OwnBeaconCommitteeMembers {
    ...
    pub async fn get_or_init_at_slot<P: Preset, W: Wait + Sync>(
        ...
        let local = controller.zip(beacon_state);

        let dependent_root = match local {
            Some((controller, state)) => {
                // The prefetch's finalized indices, not the head's: a validator only in the head
                // cannot have duties yet, and a differing list would clear the cache every slot.
        ...
        if needs_to_compute && let Some((_, state)) = local {
            self.init_at_slot(state, dependent_root, slot, validator_indices)
                .await?;
        }
    ...
    pub async fn init_at_slots<P: Preset, W: Wait + Sync>(
        ...
        for (_, slots) in slots_by_epoch::<P>(slots) {
            ...
            // A fetch that ran long can land after a reorg moved the root; the nodes' current one
            // decides, so that a slower task from an earlier slot cannot replace newer duties.
            let epoch = misc::compute_epoch_at_slot::<P>(slots.start);

            if let Some(validator_index) = validator_indices.first().copied()
                && beacon_nodes.dependent_root(epoch, validator_index).await? != dependent_root
            {
                debug_with_peers!(
                    "discarding attester duties for epoch {epoch} under {dependent_root:?}, \
                     which is no longer the dependent root",
                );
    ...
    async fn init_from_duties<P: Preset>(
        ...
        for (duty, selection_proof) in duties.into_iter().zip(selection_proofs) {
            ...
            // `own_singular_attestations` sizes and indexes a `BitList` by these pre-Electra, and
            // `BitList` panics outside its bounds.
            if validator_committee_index >= committee_length
                || committee_length > P::MaxValidatorsPerCommittee::USIZE
            {
                warn_with_peers!(
                    "skipping attester duty of validator {validator_index} in slot {slot}: \
                     beacon node reported position {validator_committee_index} \
                     in a committee of {committee_length}",
                );
                continue;
            }
...
mod tests {
    ...
    // A position at or past the committee length would panic when the attestation bitlist is set.
    #[tokio::test]
    async fn skips_a_duty_whose_position_is_outside_the_committee() -> Result<()> {
        let (own_members, state) = own_members_with_keys()?;
        ...
        own_members
            .init_from_duties::<Minimal>(
                ...
                misc::slots_in_epoch::<Minimal>(0)?,
                vec![
                    duty(public_key(40), 40, 1, 4, 1),
                    duty(public_key(41), 41, 0, 0, 1),
                ],
            )
            .await?;
        ...
        let members_at_slot_1 = own_members
            .get_at_slot::<Minimal>(1)
            .await
            .expect("the valid duty should still be kept");

        assert_eq!(members_at_slot_1.len(), 1);
        assert_eq!(members_at_slot_1[0].validator_index, 41);
