...
impl<P: Preset, W: Wait> ChainSource<P, W> {
    ...
    pub fn genesis_time(&self) -> UnixSeconds {
        match self {
            Self::Local { controller, .. } => controller.genesis_time(),
    ...
    pub fn slot(&self) -> Slot {
        match self {
            Self::Local { controller, .. } => controller.slot(),
            Self::Remote {
                chain_config,
                genesis_time,
                ..
    ...
    pub const fn controller(&self) -> Option<&ApiController<P, W>> {
        match self {
            Self::Local { controller, .. } => Some(controller),
            Self::Remote { .. } => None,
        }
    }
    ...
    pub const fn block_producer(&self) -> Option<&Arc<BlockProducer<P, W>>> {
        match self {
            Self::Local { block_producer, .. } => Some(block_producer),
            Self::Remote { .. } => None,
        }
    }
    ...
    pub const fn event_channels(&self) -> Option<&Arc<EventChannels<P>>> {
        match self {
            Self::Local { event_channels, .. } => Some(event_channels),
            Self::Remote { .. } => None,
        }
    }
