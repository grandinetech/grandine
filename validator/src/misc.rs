...
impl<P: Preset, W: Wait> ChainSource<P, W> {
    pub async fn validator_indices(
        ...
        // From the finalized state, so that a reorg cannot change an index once resolved.
        match &self.chain {
            Chain::Local(local) => {
    ...
    #[must_use]
    pub fn builder_api(&self) -> Option<&Arc<BuilderApi>> {
        self.local().and_then(|local| local.builder_api.as_ref())
    }

    #[must_use]
    pub fn registers_with_builder(&self) -> bool {
        match &self.chain {
            Chain::Local(local) => local.builder_api.is_some(),
            Chain::Remote(remote_beacon_nodes) => remote_beacon_nodes.use_builder(),
        }
    }
