...
/// The beacon nodes a slot's duties are performed against: the built-in one, or the serving
/// remote ones when there is none.
...
impl<P: Preset, W: Wait + Sync> BeaconNodes<P, W> {
    pub fn remote(remotes: &RemoteBeaconNodes, current_slot: Slot, max_empty_slots: u64) -> Self {
        Self::Remote(RemoteNodes {
    ...
    pub async fn attester_duties_at_slots(
        ...
        match self {
            Self::Local(node) => {
                node.attester_duties_at_slots(slots, validator_indices)
                    .await
            }
            Self::Remote(remotes) => {
    ...
    pub async fn head_block_root(&self) -> Result<H256> {
        match self {
            ...
            Self::Remote(remotes) => remotes.head_block_root().await,
        }
    }
}
...
impl RemoteNodes {
    ...
    fn spawn_broadcast<F, Fut>(&self, operation: &'static str, attempt: F)
    where
        F: Fn(Arc<RemoteBeaconNode>) -> Fut + Send + 'static,
        Fut: Future<Output = Result<()>> + Send,
    {
        let nodes = self.serving_nodes();

        if !nodes.is_empty() {
            spawn_broadcast(operation, nodes, attempt);
        }
    }
}
...
impl<P: Preset, W: Wait + Sync> BeaconNodeApi<P> for BeaconNodes<P, W> {
    async fn liveness(
        ...
        let remotes = match self {
            Self::Local(node) => return node.liveness(epoch, validator_indices).await,
            Self::Remote(remotes) => remotes,
        };
        ...
        let responses = join_all(remotes.nodes.iter().map(|node| async move {
    ...
    async fn publish_singular_attestations(
        ...
        match self {
            ...
            Self::Remote(remotes) => {
                remotes.spawn_publish("publish attestations", PublishedDuty::Attestations, || {
    ...
    async fn subscribe_to_beacon_committees(
        ...
        match self {
            ...
            Self::Remote(remotes) => {
                ...
                remotes.spawn_broadcast("update beacon committee subscriptions", move |node| {
    ...
    async fn validator_indices(
        ...
        let remotes = match self {
            Self::Local(node) => return node.validator_indices(public_keys).await,
            Self::Remote(remotes) => remotes,
        };
            ...
            Self::Remote(remotes) => {
                for node in &remotes.nodes {
                    match BeaconNodeApi::<P>::slot_head(node.as_ref(), slot).await {
                        ...
                        Err(error) => {
                            warn_with_peers!(
                                "{node} beacon node reported an unusable head: {error:?}"
                            );
                        }
                    }
                }
    ...
    async fn produce_block(
        ...
        match self {
            ...
            Self::Remote(remotes) => {
                let (producer, block) = remotes
                    .first_success("produce a block", |node| {
                ...
                remotes.producer.get_or_init(|| producer.clone_arc());
    ...
    async fn publish_block(
        ...
        match self {
            ...
            Self::Remote(remotes) => {
                remotes
                    .publish_from_producer("publish a block", |node| async move {
    ...
    async fn publish_blinded_block(
        ...
        match self {
            ...
            Self::Remote(remotes) => {
                remotes
                    .publish_from_producer("publish a blinded block", |node| async move {
            ...
            Self::Remote(remotes) => {
                remotes
                    .publish_from_producer(
                        "publish an execution payload envelope",
                        |node| async move {
    ...
    async fn payload_attestation_data(&self, slot: Slot) -> Result<Option<PayloadAttestationData>> {
        let remotes = match self {
            Self::Local(node) => return node.payload_attestation_data(slot).await,
            Self::Remote(remotes) => remotes,
        };
                ...
                remotes.spawn_publish(
                    ...
                    || {
                        ...
                        move |node: Arc<RemoteBeaconNode>| {
                            let messages = owned.clone_arc();
                            async move {
                                BeaconNodeApi::<P>::publish_payload_attestations(
                                    node.as_ref(),
                                    &messages,
                                )
                                .await
    ...
    async fn prepare_beacon_proposer(&self, proposers: &[ProposerData]) -> Result<()> {
        ...
        match self {
            ...
            Self::Remote(remotes) => {
                ...
                remotes.spawn_broadcast("prepare beacon proposers", move |node| {
    ...
    async fn register_validators(
        ...
        match self {
            ...
            Self::Remote(remotes) => {
                ...
                remotes.spawn_broadcast("register validators", move |node| {
                ...
                remotes
                    .publish(
                        ...
                        move |node: Arc<RemoteBeaconNode>| {
                            ...
                            async move {
                                BeaconNodeApi::<P>::publish_proposer_preferences(
                                    node.as_ref(),
                                    &preferences,
                                )
                                .await
