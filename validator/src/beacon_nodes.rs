    ...
    async fn produce_block(
        ...
        let attempt = self
            .first_success("produce a block", |node| async move {
                ...
                let shares_head = node
                    .chain_head()
                    .cached()
                    .is_some_and(|(_, head)| head == parent_root);

                if built_on != parent_root && !shares_head {
    ...
    Fut: Future<Output = Result<()>>,
{
    ...
    for node in &remotes {
        match attempt(node.clone_arc()).await {
            Ok(()) => return Ok(()),
            Err(error) => {
                warn_publish_failure(node, operation, &error);
    ...
    Fut: Future<Output = Result<()>>,
{
    let results = remotes
        ...
        .map(|node| {
            ...
            async move {
                attempt
                    .await
                    .inspect_err(|error| warn_publish_failure(node, operation, error))
            }
        })
        .pipe(join_all)
        .await;
    ...
    if results.iter().all(Result::is_err) {
        ...
        return Err(publish_error(operation, timed_out));
    }

    Ok(())
}

fn warn_publish_failure(node: &RemoteBeaconNode, operation: &str, error: &Error) {
    if is_timeout(error) {
