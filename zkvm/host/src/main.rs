...
use crate::backend::{ProofTrait, ReportTrait, ConfigKind};
    ...
    match args.command {
        Command::Execute => {
            ...
            let (output_bytes, report) =
                vm.execute(selected_test.config, state_ssz, block_ssz, cache, vec![phase_byte])?;
        ...
        Command::Prove => {
            let started_at = Instant::now();
            let vm = Vm::new()?;
            let (output_bytes, proof) = vm.prove(selected_test.config, state_ssz, block_ssz, cache, vec![phase_byte])?;
            let state_root = H256(output_bytes.try_into().unwrap());
            println!("elapsed: {:?}", started_at.elapsed());
            println!("state root after state transition: {:?}", state_root);
