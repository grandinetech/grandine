...
pub fn main() {
    ...
    let (config, block, mut state, cache) =
        read_block_and_state::<Mainnet>(&input).unwrap();
...
fn read_block_and_state<P: Preset>(
    ...
    let (config_kind, state_ssz, block_ssz, cache_ssz, phase_bytes): (u8, Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>) = bincode::deserialize(input).unwrap();
