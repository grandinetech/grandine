...
const CASE: Case = Case {
    case_path_relative_to_workspace_root: "eth2-cache/withdrawals_devnet_4",
};
...
pub static GENESIS_BEACON_BLOCK: LazyBeaconBlock<Mainnet> =
    LazyBeaconBlock::new(|| generic::beacon_block(&Config::withdrawal_devnet_4(), CASE, 0, 6));
...
pub fn beacon_blocks(
    ...
    generic::beacon_blocks(&Config::withdrawal_devnet_4(), CASE, slots, width)
}

#[must_use]
pub fn beacon_state(slot: Slot, width: usize) -> Arc<BeaconState<Mainnet>> {
    generic::beacon_state(&Config::withdrawal_devnet_4(), CASE, slot, width)
}
