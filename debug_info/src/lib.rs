...
impl HealthCheck {
    ...
    pub fn check(&mut self) {
        ...
        if let Some(metrics) = self.metrics.as_ref() {
            ...
            let report_duration = if duration_since_last_check
                > HEALTH_CHECK_INTERVAL_DURATION + Duration::from_millis(1)
            {
                duration_since_last_check
            } else {
                Duration::ZERO
            };

            metrics.set_service_delay(&self.service_name, report_duration);
        }
