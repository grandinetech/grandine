...
impl<P: Preset> Pool<P> {
    ...
    // `attestation_data_root` is the spec root and `committee_index` is supplied separately.
    // Both are matched directly against the `AttestationKey` fields.
