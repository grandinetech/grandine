    ...
    let target_epoch_participation = match pd {
        ...
        ParticipationDiff::NonZero { updated, extension } => {
            for update in updated {
                *base_epoch_participation
                    .get_mut(update.0 as u64)
                    .expect("Failed to index in base_epoch_participation") = update.1;
            }
            for element in extension {
                base_epoch_participation
                    .push(element)
                    .expect("Failed to push to base_epoch_participation");
            }
            base_epoch_participation
        }
    };
