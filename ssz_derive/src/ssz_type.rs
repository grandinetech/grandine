...
impl SszType {
    ...
    fn unify_fn_impl(&self, ssz: &Path) -> Result<ImplItemFn, Error> {
        ...
        let stmts = self.unskipped_fields()?.map(|(member, _)| {
            quote! {
                // Do not use the `&&` operator.
                // It short-circuits, preventing unification of later fields.
                equal &= #ssz::SszUnify::unify(&mut self.#member, &other.#member);
            }
        });
