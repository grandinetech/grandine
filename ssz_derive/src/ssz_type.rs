...
use syn::{
    Error, Expr, ExprLit, Generics, Ident, ImplGenerics, ImplItemFn, ImplItemType, Lit, Member,
    Path, TypeGenerics, TypeParam, WhereClause, WherePredicate, parse_quote,
...
pub struct SszType {
    ...
    transparent: bool,
    #[darling(default)]
    stable: Option<StableContainer>,
}

#[derive(FromMeta)]
struct StableContainer {
    active: ActiveFields,
}

struct ActiveFields(Vec<bool>);

impl FromMeta for ActiveFields {
    fn from_expr(expr: &Expr) -> darling::Result<Self> {
        fn parse_bit(elem: &Expr) -> darling::Result<bool> {
            let Expr::Lit(ExprLit {
                lit: Lit::Int(int), ..
            }) = elem
            else {
                return Err(
                    darling::Error::custom("`active` entries must be literals 0 or 1")
                        .with_span(elem),
                );
            };

            match int.base10_parse::<u16>() {
                Ok(0) => Ok(false),
                Ok(1) => Ok(true),
                _ => Err(darling::Error::custom("must be either 0 or 1").with_span(elem)),
            }
        }
        ...
        let bits = match expr {
            ...
            Expr::Repeat(repeat) => {
                ...
                let Expr::Lit(ExprLit {
                    ...
                    return Err(darling::Error::custom(
                        "`active` repeat length must be an integer literal",
                    )
                    .with_span(&repeat.len));
                };
            ...
            _ => {
                return Err(
                    darling::Error::custom("`active` must be an array literal").with_span(expr)
                );
            }
        };
...
impl SszType {
    ...
    fn validate(&self) -> Result<(), Error> {
        ...
        if let Some(stable) = &self.stable {
            if self.transparent {
                return Err(Error::new(
                    Span::call_site(),
                    "stable containers cannot be transparent",
                ));
            }

            let active = stable.active.0.as_slice();

            if !(1..=256).contains(&active.len()) {
                return Err(Error::new(
                    Span::call_site(),
                    "`active` must have between 1 and 256 entries",
                ));
            }

            if active.last() == Some(&false) {
                return Err(Error::new(Span::call_site(), "`active` must not end in 0"));
            }

            let active_count = active.iter().filter(|active| **active).count();
            let field_count = self.unskipped_fields()?.count();

            if active_count != field_count {
                return Err(Error::new(
                    Span::call_site(),
                    format!(
                        "number of active fields ({active_count}) must equal \
                         number of struct fields ({field_count})"
                    ),
                ));
            }
        }
    ...
    fn hash_tree_root_fn_impl(&self, ssz: &Path) -> Result<ImplItemFn, Error> {
        if let Some(stable) = &self.stable {
            let members = self.unskipped_fields()?.collect::<Vec<_>>();
            ...
            // TODO(gloas): this can be optimized, by inlining merkleization,
            // instead of calling `merkleize_progressive`.
            return Ok(parse_quote! {
                fn hash_tree_root(&self) -> #ssz::H256 {
                    let chunks = [#(#chunks),*];
                    let root = #ssz::ProgressiveMerkleTree::merkleize_progressive(chunks);
                    #ssz::mix_in_active_fields(root, #ssz::H256([#(#active_fields),*]))
                }
            });
        }
