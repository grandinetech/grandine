# PowerShell equivalent of the bash script for running Clippy
...
# Combine the predefined options with any additional arguments passed to the script
$allArgs = $options + $args

# Run cargo clippy with all arguments
& cargo clippy $allArgs

# Exit with the last exit code (equivalent to `exec` in bash)
exit $LASTEXITCODE