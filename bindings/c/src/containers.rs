...
impl TryInto<BlobsBundleV1<Mainnet>> for CBlobsBundleV1 {
    ...
    fn try_into(self) -> Result<BlobsBundleV1<Mainnet>, Self::Error> {
        Ok(BlobsBundleV1 {
            ...
            proofs: ContiguousList::try_from_iter(self.proofs.into_iter().map(Into::into))?,
            blobs: ContiguousList::try_from_iter(self.blobs.into_iter().map(Into::into))?,
        })
    }
}
...
impl<N: ArrayLength<u8>> Into<ByteVector<N>> for CVec<u8> {
    fn into(self) -> ByteVector<N> {
        ByteVector::from(self.as_bytes())
    }
}

impl Into<Blob<Mainnet>> for CVec<u8> {
