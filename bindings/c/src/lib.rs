...
impl eth1_api::EmbedAdapter for CEmbedAdapter {
    ...
    fn engine_new_payload_v4(
        ...
        let versioned_hashes = versioned_hashes
            ...
            .collect::<CVec<_>>();
        let parent_beacon_block_root = parent_beacon_block_root.into();
        let execution_requests = execution_requests.into();
    ...
    fn engine_get_blobs_v2(
        ...
        result.map(|blobs| {
            ...
            Some(blobs.into_iter().map(Into::into).collect())
        })
    }
}
...
pub extern "C" fn grandine_run(argc: u64, argv: *const *const c_char) -> u64 {
    fn try_run(argc: u64, argv: *const *const c_char) -> Result<()> {
        let args = unsafe {
            std::iter::once("").chain(
