...
impl_c_result!(CResultCLogs, CLogs);
...
struct CLog {
    ...
    topics: CVecCH256,
    data: CVecU8,
    block_hash: COptionCH256,
    block_number: COptionU64,
    transaction_hash: COptionCH256,
    transaction_index: COptionU64,
    log_index: COptionCH256,
    transaction_log_index: COptionCH256,
    log_type: COptionCStr,
    removed: COptionBool,
}
    ...
    fn into(self) -> Vec<Log> {
        ...
        raw_slice
            ...
            .map(|log| {
                ...
                let log_type: Option<CCharPtr> = log.log_type.clone().into();
                ...
                Log {
                    ...
                    log_type: log_type.map(|str| {
                        unsafe { CStr::from_ptr(str.0) }
                            .to_str()
                            .unwrap()
                            .to_string()
                    }),
...
pub struct CEmbedAdapter {
    eth_block_number: unsafe extern "C" fn() -> CResultU64,
    eth_get_block_by_hash: unsafe extern "C" fn(hash: CH256) -> CResultCOptionCEth1Block,
    eth_get_block_by_number: unsafe extern "C" fn(number: u64) -> CResultCOptionCEth1Block,
    eth_get_block_finalized: unsafe extern "C" fn() -> CResultCOptionCEth1Block,
    eth_get_block_safe: unsafe extern "C" fn() -> CResultCOptionCEth1Block,
...
impl eth1_api::EmbedAdapter for CEmbedAdapter {
    ...
    fn eth_get_block_by_hash(&self, hash: H256) -> Result<Option<eth1_api::Eth1Block>> {
        <CResultCOptionCEth1Block as Into<Result<_>>>::into(unsafe {
            (self.eth_get_block_by_hash)(CH256(hash.0))
        })
        .map(|v| <COptionCEth1Block as Into<Option<_>>>::into(v).map(|v: CEth1Block| v.into()))
    }
    ...
    fn eth_get_block_by_number(&self, number: BlockNumber) -> Result<Option<eth1_api::Eth1Block>> {
        ...
        <CResultCOptionCEth1Block as Into<Result<_>>>::into(output)
            .map(|v| <COptionCEth1Block as Into<Option<_>>>::into(v).map(|v: CEth1Block| v.into()))
    }
