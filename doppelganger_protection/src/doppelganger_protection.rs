...
impl DoppelgangerProtection {
    ...
    fn activate_validators_that_pass_checks<P: Preset>(
        ...
        let validators_to_activate = self
            .load()
            .tracked_validators
            .iter()
            .map(|(public_key, added_in_slot)| (*public_key, *added_in_slot))
            .filter(|(public_key, added_in_slot)| {
        ...
        self.update(|snapshot| {
            ...
            snapshot.active_validators.extend(&validators_to_activate);

            // Only the activated keys leave, so a key tracked since the load above stays.
            snapshot
                .tracked_validators
                .retain(|public_key, _| !validators_to_activate.contains(public_key));
            ...
            match indices_by_pubkey.get(public_key).copied().flatten() {
                ...
                None => {
                    if added_in_slot.is_some() {
                        warn_with_peers!(
                            "liveness of validator with public key {public_key:?} cannot be \
                             checked until its index is known; it will not perform duties",
                        );
