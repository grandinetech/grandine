...
The Validator Client is a built-in component that is activated if validator keys are passed to Grandine. Let's try running Grandine with a new validator enabled on the Holesky network (assuming you already have keys, secrets etc.):
...
* `--keystore-password-file` - a file containing a single secret for all the validator keystore files (this option is usable if all the keystores are secured with the same secret);
* `--keystore-password-dir` - a directory containing secrets for all the validator keystore files (this option is usable if all the keystores are secured with different secrets), for every keystore file in `--keystore-dir` there should be a corresponding file in the `--keystore-password-dir` directory with the secret file named the same as the corresponding keystore file except the extension should be `.txt` instead of `.json`.
