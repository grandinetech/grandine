...
use types::{
    ...
    gloas::{
        ...
        containers::{
            ...
            ProposerPreferences, SignedExecutionPayloadBid, SignedExecutionPayloadEnvelope,
        },
...
use crate::{
    ...
    misc::{
        ...
        PayloadAttestationItem, PayloadAttestationValidationError, PayloadPresence, Score,
...
impl<P: Preset, S: Storage<P>> Store<P, S> {
    ...
    #[instrument(level = "debug", skip_all)]
    pub fn validate_gloas_data_column_sidecar(
        &self,
    ...
    pub fn validate_data_column_sidecar(
        ...
        match Arc::unwrap_or_clone(data_column_sidecar) {
            ...
            DataColumnSidecar::Gloas(data_column_sidecar) => self
                .validate_gloas_data_column_sidecar(
                    data_column_sidecar,
                    block_seen,
                    origin,
                    validate_block_presence,
    ...
    fn propagate_and_dissolve_differences(
        ...
        while let Some(last_in_segment) = difference_queue.pop() {
            ...
            while let Some(current) = difference_queue.peek_mut() {
                ...
                previous.differences.pending = previous.differences.pending.saturating_add(current.differences.pending);
            }
