...
use crate::{backend::Backend, error::Error, PublicKey};
...
impl CachedPublicKey {
    ...
    pub fn decompress(&self, backend: Backend) -> Result<&PublicKey, Error> {
        self.decompressed.get_or_try_init(|| match backend {
            Backend::Blst => PublicKey::try_from_with_backend(self.bytes, backend).map(Box::new),
        })
    }
}
